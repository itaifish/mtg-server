...
use crate::engine::actions::{cast_spell, pass_priority, activate_mana_ability};
...
fn setup_soul_warden_pridemate_game() -> GameState {
    ...
    let deck = vec![
        DeckEntry { card_name: "Plains".into(), count: 20 },
        DeckEntry { card_name: "Soul Warden".into(), count: 4 },
        DeckEntry { card_name: "Ajani's Pridemate".into(), count: 4 },
        DeckEntry { card_name: "Savannah Lions".into(), count: 2 },
    ];
    load_deck(&mut state, "alice", &deck).unwrap();
    load_deck(&mut state, "bob", &vec![
        DeckEntry { card_name: "Plains".into(), count: 30 },
    ]).unwrap();
...
fn soul_warden_triggers_on_creature_etb() {
    ...
    state.player_zones.get_mut("alice").unwrap().hand.insert(901);
    ...
    assert_eq!(state.pending_triggers[0].description,
...
fn pridemate_triggers_on_life_gain_from_soul_warden() {
    ...
    let sw = crate::game::card::CardInstance::new(
        900, "alice", card_by_name("Soul Warden").unwrap().clone(),
    );
    let pm = crate::game::card::CardInstance::new(
        901, "alice", card_by_name("Ajani's Pridemate").unwrap().clone(),
    );
    state.objects.insert(900, sw);
    ...
    let lion = crate::game::card::CardInstance::new(
        902, "alice", card_by_name("Savannah Lions").unwrap().clone(),
    );
    state.objects.insert(902, lion);
    state.player_zones.get_mut("alice").unwrap().hand.insert(902);

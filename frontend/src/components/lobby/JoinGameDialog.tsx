...
export function JoinGameDialog({ gameId, onClose }: JoinGameDialogProps) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    setError('');
    joinGame(client, gameId, playerName.trim(), finalDecklist);
  };
    ...
    <Modal title="Join Game" onClose={onClose}>
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
          <label htmlFor="join-decklist" style={{ fontSize: '0.85rem', color: 'var(--color-text-muted)' }}>Or paste a decklist</label>
          <textarea id="join-decklist" value={deckText} onChange={(e) => { setDeckText(e.target.value); setDecklist([]); }} placeholder={'4 Lightning Bolt\n4 Mountain'} rows={6}
